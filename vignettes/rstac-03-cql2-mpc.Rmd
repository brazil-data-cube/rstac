...
df <- items_as_sf(stac_items)  |>
  dplyr::mutate(cloud_cover = rstac::items_reap(stac_items, c("properties", "eo:cloud_cover"))) |> 
  dplyr::mutate(datetime = as.Date(datetime)) |>
  dplyr::group_by(datetime) |>
  dplyr::summarise(cloud_cover = mean(cloud_cover)) |>
  dplyr::mutate(cloud_cover = slider::slide_mean(cloud_cover, before = 3, after = 3))

df |> 
  ggplot2::ggplot() +
  ggplot2::geom_line(ggplot2::aes(x = datetime, y = cloud_cover))
```
