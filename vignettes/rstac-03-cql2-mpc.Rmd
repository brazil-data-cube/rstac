...
```{r cql2-search, eval=not_on_cran}
...
```{r items-length, eval=not_on_cran}
stac_items
```
...
```{r geojson-to-sf, eval=not_on_cran}
...
```{r lowest-cloud-cover, eval=not_on_cran}
...
```{r assets-list, eval=not_on_cran}
...
```{r asset-preview, eval=not_on_cran, fig.height=3, fig.width=5}
...
```{r cql2-search-cloud, eval=not_on_cran}
stac_items <- planetary_computer |>
  ext_filter(
...
```{r assets-rename, eval=not_on_cran}
...
```{r items-fetch, eval=not_on_cran}
stac_items <- planetary_computer |>
  ext_filter(
...
```{r cloud-cover-ts-plot, eval=not_on_cran}
library(dplyr)
library(slider)
library(ggplot2)
...
```{r collection-landsat-bands, eval=not_on_cran}
...
```{r landsat-assets, eval=not_on_cran}
...
```{r collection-daymet, eval=not_on_cran}
...
```{r daymet-assets, eval=not_on_cran}
items_assets(daymet)
