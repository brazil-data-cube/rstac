...
We have submitted `rstac` 0.9.2 version to Comprehensive R Archive Network (CRAN), now R users can install the package directly from CRAN. An important step to promote new `rstac` users is a well-documented package and tutorials. In this `rstac` version, we have updated the package documentation and write new tutorials. Firtly, we have prepared a new R notebook on how to create CQL2 expression using `rstac`. We plan to submit it to Microsoft Planetary Computer Examples repository [https://github.com/microsoft/PlanetaryComputerExamples] as soon as possible. We also prepared a new vignette in `rstac` to help R users to see actual examples on using CQL2 expressions.
