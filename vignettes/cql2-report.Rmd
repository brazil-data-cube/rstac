...
Earth observation images are one of the most reliable and consistent ways to monitor Earth's surface. In the last decades, the number of orbital instruments for Earth observation has increased [@finer_combating_2018]. Cloud computing has been adopted by industries and academies to store, access, process, and distribute big Earth observation data [@wagemann_user_2021]. In this context, Spatio-temporal Assets Catalog (STAC) [https://stacspec.org] is becoming a standard to describe and search satellite imagery service providers. 

STAC is an OGC API Features (OAFeat) [@ogc_ogc_2019] compliant standard. STAC enables users to search and access satellite images using a client. One of the main characteristics of STAC is its core simplicity, which supports basic search functionality. The standard also enables the development of new extensions to support more specific and complex functionality, for example, the Common Query Language 2 (CQL2) extension [@ogc_common_2022].

CQL2 is part of the OAFeat standard that provides a complex way to search and retrieve spatial-temporal assets. Using CQL2 in a STAC service, users can search for spatial assets using various metadata information such as cloud cover and spatial relations. CQL2 is a powerful query language that allows users to create complex filtering criteria in an expressive way.

In the R language, the rstac package is the unique client to support STAC services [@simoes_rstac_2021]. rstac provides basic functionality to STAC core. This report shows the final status of CQL2 extension development in rstac. It also describes the activities realized during the project and how CQL2 works in the rstac package. The current version of the rstac package can be accessed at https://github.com/brazil-data-cube/rstac.

# Project status

This section reports the final project status on developing CQL2 capability in the rstac package. We describe each project's milestones in the following topics.

## Milestone 1 - Proof of concept version with examples that validate the idea of using the package

Status: concluded. 

We used the abstract syntax tree (AST) of R language to develop the CQL2 extension in rstac. In computer science, AST is a way to represent a language expression in a formal way using data structures. AST representation informs the precedence on which and how an identifier will be evaluated. AST can be used to translate code from one language to another [@wickham_hadley_advanced_2019]. The advantage of this approach is that users can focus on filtering criteria putting less effort into checking the syntax consistency. Using R environments and lexical scoping, AST gave us a powerful way for translating R code into CQL2 query. This approach showed to be consistent with the CQL2 standard. We provide some examples of code translation in Appendix A.
...
One of the main tasks we dedicated effort to in this project was implementing integration tests. CQL2 standard provides a comprehensive conformance test [@ogc_common_2022]. We have implemented most of the tests targeting the test dataset. The test dataset used in rstac conformance tests is provided by "Test Dataset for the CQL2 Test Suite" [https://cql2test.ldproxy.net/ne110m4cql2]. It is derived from Natural Earth [https://www.naturalearthdata.com], a public domain dataset containing features from three original data layers at scale 1:110 million: `ne_110m_admin_0_countries`, `ne_110m_populated_places_simple`, and `ne_110m_rivers_lake_centerlines`. Besides this test suite, we also have implemented tests using Microsoft Planetary Computer STAC service [https://planetarycomputer.microsoft.com/api/stac/v1]. These tests were critical to detect bugs and inconsistencies along with software development. The current rstac version is passing in all of them, which asserts rstac robust compliance to CQL2 standard. The list of conformance tests implemented in rstac is listed in Appendix B.
...
We pushed a pre-released version of rstac into its GitHub repository. This version passed all conformance tests referenced in the previous topic. CQL2 filtering functionality is implemented in the `ext_filter()` function. We have written function documentation available in `?ext_filter()` in the R console. Besides function description and parameters details, the documentation provides examples of using the CQL2 filtering function using the rstac package in actual STAC services.

## Milestone 4 - CRAN submission and tutorial creation for users and developers.

Status: concluded.

We have submitted the `rstac` 0.9.2 version to Comprehensive R Archive Network (CRAN). Now R users can install the package directly from CRAN. A well-documented package and tutorials are important to promote new `rstac` users. In this `rstac` version, we have updated the package documentation and written new tutorials. Firstly, we have prepared a new R notebook on creating CQL2 expressions using `rstac`. We plan to submit it to the Microsoft Planetary Computer Examples repository [https://github.com/microsoft/PlanetaryComputerExamples] as soon as possible. We also prepared a new vignette in `rstac` to help R users see actual examples of CQL2 expressions.

# Concluding remarks

This report describes the results of the CQL2 extension development in the `rstac` package. The project enumerated four milestones that were concluded as described above. The current version of the `rstac` package can be accessed at [https://github.com/brazil-data-cube/rstac] under the MIT license. The `rstac` package is open to the community and follows the "Contributor Covenant - version 1.0.0" code of conduct. We updated the package's documentation and have prepared new tutorials on how to use `rstac` and its new CQL2 functionality.
...
Here, we list the conformance tests implemented in the rstac test suite. The tests run against "Test Dataset for the CQL2 Test Suite" [https://cql2test.ldproxy.net/ne110m4cql2] STAC service. rstac implements all conformance tests that aim to test predicates against the test dataset (except for conformance test #22, which is not present in "Test Dataset for the CQL2 Test Suite").
