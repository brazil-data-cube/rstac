...
```{r item-collection, eval=not_on_cran}
s_obj %>% 
...
- **`items_filter()`:** Performs a filter by items according to expressions operating on the properties of a `Items` object.
...
- **`items_assets()`:** Returns the assets name from `Items` and `Item` objects.


It is interesting to verify the fields of items before filtering:

```{r fields, eval=not_on_cran}
s_obj %>%
  stac_search(
    collections = "CB4-16D-2",
...
```{r assets-select, eval=not_on_cran}
...
```{r assets-renamed, eval=not_on_cran}
...
```{r plot-preview, eval=not_on_cran, fig.height=3, fig.width=5}
